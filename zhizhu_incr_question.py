...
def get_url():
    ...
    """
    url_list = []
    id_count = int(Rs.llen("zhihu:question:id"))
    if id_count >= 100:
        for i in range(100):
            article_id = Rs.rpop("zhihu:question:id")
            url = f"https://www.zhihu.com/question/{article_id}"
            url_list.append(url)
    elif id_count>0:
        for i in range(id_count):
            article_id_q = Rs.rpop("zhihu:question:id")
            if article_id_q is not None:
                article_id = article_id_q
                url = f"https://www.zhihu.com/question/{article_id}"
                url_list.append(url)
    else:
        last_id = Rs.get('zhihu:question:last_id')
        incr_id(int(last_id))
        for i in range(100):
            article_id = Rs.rpop("zhihu:question:id")
            url = f"https://www.zhihu.com/question/{article_id}"
...
def incr_id(value):
    for i in range(1000):
        Rs.lpush("zhihu:question:id", value)
        value += 1
    ...
    try:
        ...
        if page_content:
            if page_content.status_code == 200:
                ...
                data = parse_html(ret, question_id)
                save_data(data, url)
    except Exception as e:
        print("error message:" + str(e))


def parse_html(response, question_id):
    '''
    解析html,获取各个字段的值
    ...
    try:
        ...
        article_content = html.xpath("//span[@class='RichText ztext']//text()")
        ...
        time_stamp = content_html["initialState"]["entities"]["questions"][question_id]["created"]
...
def save_data(data, url):
    ...
    article_detail_data['website_no'] = "S18465"
    ...
    Rs.incr('counter:zhihu:question:%s' % today_date)
...
def main():
    ...
    logging.basicConfig(filename="debug_q.log", filemode="w", format="%(asctime)s %(name)s:%(levelname)s:%(message)s",
                        datefmt="%d-%M-%Y %H:%M:%S", level=logging.DEBUG)
    while True:
        try:
            result_count_1 = int(Rs.get('counter:zhihu:question:%s' % today_date))
        except:
            Rs.set('counter:zhihu:question:%s' % today_date, 0)
            result_count_1 = int(Rs.get('counter:zhihu:question:%s' % today_date))
        for i in range(10):
            ...
            Rs.set('zhihu:question:last_id', last_article_id)
