...
def get_url():
    ...
    """
    url_list = []
    id_count = int(Rs.llen("zhihu:zhuanlan:id"))
    if id_count >= 100:
        print(int(id_count))
        for i in range(100):
            article_id_a = Rs.rpop("zhihu:zhuanlan:id")
        ...
        last_id = Rs.get('zhihu:zhuanlan:last_id')
        incr_id(int(last_id))
        for i in range(100):
            article_id = Rs.rpop("zhihu:zhuanlan:id")
            url = f"https://zhuanlan.zhihu.com/p/{article_id}"
            url_list.append(url)
...
def incr_id(value):
    for i in range(1000):
        Rs.lpush("zhihu:zhuanlan:id", value)
        value += 1
    ...
    try:
        ...
        if page_content:
            if page_content.status_code == 200:
                zhuanlan_id = url.split('/')[-1]
                ret = page_content.text
                data = parse_html(ret, zhuanlan_id)
                save_data(data, url)
    except Exception as e:
        print("error message:" + str(e))


def parse_html(response, zhuanlan_id):
    '''
    解析html,获取各个字段的值
    ...
    try:
        ...
        data["article_title"] = html.xpath("//h1[@class='Post-Title']/text()")[0]
        article_content = html.xpath("//div[@class='RichText ztext Post-RichText']//text()")
        ...
        time_stamp = content_html["initialState"]["entities"]["articles"][zhuanlan_id]["created"]
...
def save_data(data, url):
    ...
    article_detail_data['website_no'] = "S17879"
    ...
    Rs.incr('counter:zhihu:zhuanlan:%s' % today_date)
...
def main():
    ...
    while True:
        try:
            result_count_1 = int(Rs.get('counter:zhihu:zhuanlan:%s' % today_date))
        except:
            Rs.set('counter:zhihu:zhuanlan:%s' % today_date, 0)
            result_count_1 = int(Rs.get('counter:zhihu:zhuanlan:%s' % today_date))
        # for循环开始，取1000个ID
        for i in range(10):
            ...
            try:
                Rs.set('zhihu:zhuanlan:last_id', last_article_id)
            except Exception as e:
                print(url_info)
